...
export async function updateTacheInter(newDoc, callback) {
    await updateDoc(doc(db, 'interventions', newDoc.uid), { ...newDoc, [`history.${[new Date().toLocaleString().replaceAll('/', '_')]}`]: 'Réalisation d\'une tache $$' + newDoc.technicien + '$$' });
    addReducedInfo({ ...newDoc, uid: newDoc.uid }, ({ functionName, idReducedInfo }) => { console.log('\tUpdate task - ' + functionName, idReducedInfo); });
    ...
    callback({ functionName: 'updateTacheInter', id: newDoc.uid });
    return newDoc.uid;
};
...
export async function editRatioUser(newDoc, callback) {
    const col = doc(db, 'users', newDoc.technicien);
    await updateDoc(col, {
        [`completionInter.${newDoc.date.split(' ')[0].replaceAll('/', '_')}`]: newDoc.rate
    });

    callback({ functionName: 'editRatioUser', id: newDoc.technicien });
    return 12;
};
export async function addCagnotteUser(newDoc, callback) {
    const col = doc(db, 'users', newDoc.technicien);
    await updateDoc(col, {
        [`cagnotte.${newDoc.date.split(' ')[0].replaceAll('/', '_')}`]: newDoc.nbInter
    });

    callback({ functionName: 'addCagnotteUser', id: newDoc.technicien });
    return 12;
};
export async function removeCagnotteUser(newDoc, callback) {
    const col = doc(db, 'users', newDoc.technicien);
    await updateDoc(col, {
        [`cagnotte.${newDoc.date.split(' ')[0].replaceAll('/', '_')}`]: deleteField()
    });

    callback({ functionName: 'removeCagnotteUser', id: newDoc.technicien });
    return 12;
};
